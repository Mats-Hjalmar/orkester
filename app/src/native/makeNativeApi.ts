...
// jsi-udp / RN-fetch transports never enter the web build. Calling it on web is a
// wiring bug (web uses MockApi), so it THROWS rather than silently returning a
// no-op Api.
...
export function makeNativeApi(_lock?: MulticastLock): Api {
  throw new Error(
