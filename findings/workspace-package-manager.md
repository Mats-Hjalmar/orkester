# Workspace / package-manager findings (pnpm history, now bun)
...
- 2026-06-25 (TIER-2 GUARD): The recurring failure class is "import-by-name smoke
  ...
  ```
- 2026-08-21: **Migrated the whole workspace to bun** — one build tool for installs and scripts. Deleted `pnpm-lock.yaml`, `.npmrc` (`node-linker=hoisted`) and `pnpm-workspace.yaml`; workspaces now come from the root `package.json` `workspaces` field, `packageManager` says `bun@1.3.11`, and `bun.lock` is committed. Everything above about pnpm's hoisted linker is **history**, not current config. What was verified after a clean `rm -rf node_modules && bun install` (1823 packages, 6.7s): `bun run typecheck` (core + app), the 194 core tests, a desktop `electron-vite build` + renderer-no-node guard, `electron-builder --mac dir` (signed) and — the actual worry — **Metro still resolves**: `expo export --platform ios` bundled 3.98 MB of Hermes bytecode and every asset, resolving through bun's `node_modules/.bun/<pkg>@<version>/node_modules/...` symlink store rather than a flat tree. So Metro does not need pnpm's hoisted layout specifically; it needs *a* layout where every package is reachable by symlink from `node_modules`, which bun provides. Two operational notes: workspace-package binaries land in that package's own `node_modules/.bin` (`desktop/node_modules/.bin/electron-builder`, `packages/core/node_modules/.bin/vitest`), not the root one, so run them through `bun run --cwd <pkg> <script>`; and electron-builder logs "bun does not support any CLI for dependency tree extraction, utilizing file traversal collector instead" — harmless here because the packaged app ships no `node_modules` at all.
- 2026-08-21: `bun run --cwd <dir> <script>` is the per-package form used by the root scripts. `--filter` truncates each package's output to 10 lines in a TTY (it would hide the electron-builder log) and `--elide-lines=0` is rejected outside a terminal, so `--cwd` is what the single-package targets use; `--filter` is fine for `typecheck`, where the output is a pass/fail line per package. Also: `bun test` and `bun build` are bun's own subcommands (its test runner / bundler), so the package scripts must always be invoked as `bun run test` / `bun run build`.
